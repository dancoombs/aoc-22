...
fn score(input: Input, rules: &HashMap<(&str, &str), u32>) -> u32 {
    input
...
mod tests {
    ...
    fn test_part_one() {
        ...
        assert_eq!(part_one(Input::new(&input)).unwrap(), 15);
    }

    #[test]
    fn test_part_two() {
        let input = advent_of_code::read_file("examples", 2);
        assert_eq!(part_two(Input::new(&input)).unwrap(), 12);
    }
}
